...
/// Push a `SessionUpdate` upstream via `client.notify-session`. All
/// session updates flow through this side channel; the agent direction
/// only carries the eventual `prompt-response`.
